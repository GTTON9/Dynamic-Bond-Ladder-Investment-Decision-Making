...
standard_mats_20 <- date_list[[1]]$Maturity[1:24]
std_int_knots <- as.vector(c(1/12, quantile(standard_mats, probs= c(.2,.4,.6,.8)), 30))
std_int_knots_20 <- as.vector(c(1/12, quantile(standard_mats_20, probs= c(.2,.4,.6,.8)), 20))
```
...
# over a time window
interp_yc(date_list, int_knots=std_int_knots_20, d = 1)

fit_nelson_siegel <- function(yield_list, int_knots = std_int_knots_20, lambda, 
                              times_to_m = c(1/12, 3, 6, 9, 12, 16, 20),
                              start, T_, spline = TRUE, static = TRUE){
  ...
  if(spline == TRUE){
    bs_yield <- interp_yc(yield_list, int_knots = int_knots, degree = 3, 1)
    row_idx <- which(round(bs_yield$ttm, 3) %in% round(times_to_m,3))
    maturities <- interp_yc(yield_list, int_knots = int_knots, degree = 3, 1)$ttm[row_idx]
    print(bs_yield)
  }else {
  ...
  if(spline == TRUE){
    for(t in 1:T_){
      bs_yield <- interp_yc(yield_list, int_knots = int_knots, degree = 3, 1)
      row_idx <- which(round(bs_yield$ttm, 3) %in% round(times_to_m,3))
      yield <- interp_yc(yield_list, int_knots = int_knots, degree = 3, 1)$yield[row_idx]
      Y_mat[,t] <- yield # populate the Y_mat matrix
    }
  }else{
    for(t in 1:T_){
...
compute_yield <- function(maturity, betas,lambda, cov_matrix = FALSE) {
  term1 <- betas[1]
  ...
  if(is.null(cov_matrix)){
    noise <- mvrnorm(1, mu = 0, Sigma = cov_matrix)
  }else{
    noise <- rnorm(1, mean = 0, 0.01)
  }
  yield <- term1 + term2 + term3 + noise
  return(yield)
}

generate_data<- function(T_, lambda = 0.5, GLS = FALSE) {
  # T_: time window
  beta_1_sum <- 0
  beta_2_sum <- 0
  beta_3_sum <- 0
  ...
  maturities <- seq(1/12, 20+1/12, 10/3)

  n_tenor <- length(maturities)

  if (GLS) {
    random_matrix <- matrix(rnorm(n_tenor * n_tenor), nrow = n_tenor)
    cov_matrix <- crossprod(random_matrix)  # Ensures positive definiteness
  }
  ...
  for (j in 1:T_) {  # generation for each date within the time window
    ...
    sigma <- runif(1, 0.01, 0.05) # random epsilon

    ZERO_YLD1 <- sapply(maturities, function(maturity) compute_yield(maturity, betas, lambda, cov_matrix))
  ...
  return(list(test_data = test_data, betas = betas, cov_mat = cov_matrix, sigma = sigma_avg, lambda = lambda))
}

generate_sample <- generate_data(20, lambda = 0.3, GLS = TRUE)
test_data <- generate_sample$test_data
generate_sample$cov_mat
NS_OLS <- fit_nelson_siegel(test_data,
                  lambda = 0.3,
                  start = 1,
                  T_ = 20, spline = FALSE, static = FALSE)
...
```{r}
fit_nelson_siegel_GLS <- function(yield_list, int_knots = std_int_knots, lambda, row_idx = seq(1, 241, 40), start, T_, avg_cov = NULL, spline = TRUE, static = TRUE) {
  ...
  if(spline) {
    for(t in 1:T_) {
      bs_yield_curve <- interp_yc(yield_list, int_knots = int_knots, degree = 3, t)[row_idx,]
      Y_mat[,t] <- bs_yield_curve$yield
    }
  } else {
    for(t in 1:T_) {
      Y_mat[,t] <- yield_list[[t]]$ZERO_YLD1
  ...
  for (i in 1:max_iteration) {
    ...
    if (sum((betas - betas_new)^2) < 1e-10) {
      betas <- betas_new
      break
    }
    # Update betas
    ...
    #eps <- Y_mat - fitted_yields
    
    # Update covariance matrix based on residuals
...
# random data
generate_sample <- generate_data(20, lambda = 0.5, GLS = TRUE)
test_data <- generate_sample$test_data
generate_sample$cov_mat

# example for GLS

NS_fit_GLS <- fit_nelson_siegel_GLS(
  yield_list = test_data,
  lambda = 0.5,
  start = 1,
  spline = FALSE,
  T_ = 20
